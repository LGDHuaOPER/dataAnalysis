...
public class CoordinateDao {
	...
	public int getCoordinate(Connection conn,String waferNO,String dieNumber,String device,String lot){
		String sql = "select coordinate_id from dm_wafer_coordinate_data where die_number=? and wafer_id in (select wafer_id from dm_wafer where wafer_number=? and delete_status=0  and device_number=? and lot_number=?)";
		Object[] param = new Object[]{dieNumber,waferNO,device,lot};
		List<Map<String,Object>> ls = db.queryToList(conn, sql, param);
		int coordinateId = 0;
		if(ls.size()>0){
			coordinateId = Integer.parseInt(ls.get(0).get("coordinate_id").toString());
	...
	public int getCoordinateId(Connection conn,String waferNO,String dieX,String dieY,String device,String lot){
		String sql = "select coordinate_id from dm_wafer_coordinate_data where x_coordinate=? and y_coordinate=? and wafer_id in (select wafer_id from dm_wafer where wafer_number=? and delete_status=0  and device_number=? and lot_number=?)";
		Object[] param = new Object[]{dieX,dieY,waferNO,device,lot};
		Object result = db.queryResult(conn, sql, param);
		return result==null?0:Integer.parseInt(result.toString());
	}
	 ...
	 * @return
	 */
	public int  getSubdieId(Connection conn,int coordinateId,int subdieNumber,int waferId){
		String sql = "select subdie_id from dm_wafer_subdie where coordinate_id=? and subdie_number=? and wafer_id=?";
		Object[] param = new Object[]{coordinateId,subdieNumber,waferId};
		Object result = db.queryResult(conn, sql, param);
		return result==null?0:Integer.parseInt(result.toString());
	}
	...
	public WaferMapDTO getAllDie(Connection conn,String waferNO,String device,String lot){
		WaferMapDTO wafer = new WaferMapDTO();
		String sql  = "select x_coordinate,y_coordinate,bin,coordinate_id from dm_wafer_coordinate_data where wafer_id in (select wafer_id from dm_wafer where wafer_number=? and device_number=? and lot_number=?) ";
