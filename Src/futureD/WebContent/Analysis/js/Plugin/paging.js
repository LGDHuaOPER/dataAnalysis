...
(function(root, factory) {
	...
	Paging.prototype = {
			...
			this.container.on('click', 'li.js-page-action,li.ui-pager', function(e) {
				...
				if ($(this).hasClass('js-page-action')) {
					...
					if ($(this).hasClass('js-page-prev')) {
						_this.current = Math.max(1, parseFloat(_this.current) - 1);
					}
					if ($(this).hasClass('js-page-next')) {
						_this.current = Math.min(_this.pagecount, parseFloat(_this.current) + 1);
					}
		...
		format: function() {
			...
			if (this.pagecount > 6) {
				html += '<li data-page="1" class="ui-pager pagenum">1</li>';
				if (this.current <= 2) {
					html += '<li data-page="2" class="ui-pager pagenum">2</li>';
					html += '<li data-page="3" class="ui-pager pagenum">3</li>';
				...
				if (this.current > 2 && this.current <= this.pagecount - 2) {
					...
					html += '<li data-page="' + (parseFloat(this.current) - 1) + '" class="ui-pager pagenum">' + (parseFloat(this.current) - 1) + '</li>';
					html += '<li data-page="' + parseFloat(this.current) + '" class="ui-pager pagenum">' + this.current + '</li>';
					html += '<li data-page="' + (parseFloat(this.current) + 1) + '" class="ui-pager pagenum">' + (parseFloat(this.current) + 1) + '</li>';
					html += '<li class="ui-paging-ellipse" class="ui-pager pagenum">' + this.settings.ellipseTpl + '</li>';
				} else {
					html += '<li class="ui-paging-ellipse" >' + this.settings.ellipseTpl + '</li>';
					for (var i = this.pagecount - 2; i < this.pagecount; i++) {
						html += '<li data-page="' + i + '" class="ui-pager pagenum">' + i + '</li>'
					}
				}
				html += '<li data-page="' + this.pagecount + '" class="ui-pager pagenum">' + this.pagecount + '</li>';
			} else {
		...
		bindToolbar: function() {
			var _this = this;
			var html = $('<li class="ui-paging-toolbar"><select class="ui-select-pagesize" style="display:none;"></select>到<input type="text" class="ui-paging-count"/>页<a href="javascript:void(0)" class="Gotojump">跳转</a></li>');
