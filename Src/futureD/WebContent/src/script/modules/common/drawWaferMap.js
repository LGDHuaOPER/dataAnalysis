/*bin 5000 表示未被测试的die  ，bin 5001 表示其他期间类型的die*/
//判断鼠标当前点击位置是在die上
            ...
            for (var i = this.maxRow; i >= this.minRow; i--) {
                ...
                for (var j = this.maxCol; j >= this.minCol; j--) {
                    ...
                    if (this.coordsArray.containsKey(key)) {
                        ...
                        if(subdieFlag){
                        	if(this.param){
                        		var currentSubdieList = _.assign(_.find(this.waferData.waferList, { 'parameter': this.param }).currentSubdieList,  this.waferData.otherSubdieType);
                        	}
                        	else{
                        		var currentSubdieList = _.assign(this.waferData.waferList.currentSubdieList,  this.waferData.otherSubdieType);
                        	}
                        	...
                        	console.log("this.subcoordsArray",this.subcoordsArray);
        ...
        $(can).on({
            ...
            click: function(e) {
                ...
                if (obj.curSelectedDie != null) {
                    ...
                    if (obj.curSelectedDie.filterFlag != "disabled" && obj.curSelectedDie.Bin != -1 && obj.curSelectedDie.Bin != 5001) {
        ...
        $(document).on("keydown", function(e){
            ...
            for (var i = 1; i < len; i++) {
                ...
                if (obj.coordsArra.containsKey(newkey)) {
                    if (obj.coordsArra.getValue(newkey).Bin == -1 || obj.coordsArra.getValue(newkey).Bin == 5001 || obj.coordsArra.getValue(newkey).filterFlag == "disabled") { //遇到bin值为-1或5001或者disabled的die跳过
                        continue;
                    } else {
                        break;
                    }
...
function buildColorGradation(obj) {
    ...
    if(colorOrder === true){
        ...
        colorMap.add(5001,"#FFFFFF");
        colorMap.add(5000,"#FDFDFD");
